...
class DieModifierTest {
    ...
    void negativeDieModifier() {
        ...
        assert (-5).on_each_die_if{it.is_even} instanceof DieModifier

        assert -5.on_every_die.modifier == -5
        assert (-5).on_every_die.modifier == -5
        assert -5.on_each_die_if{it.is_even}.modifier == -5
        assert (-5).on_each_die_if{it.is_even}.modifier == -5
    }

    @Test
    void useADiceRollAsCondition() {
        def modifier = 2.on_each_die_if(new DiceRollingSpec(allDice:[1,3,5]))
